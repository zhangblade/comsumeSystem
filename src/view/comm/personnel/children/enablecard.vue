	...
	export default {
		...
		methods: {
            getData(){
                ...
                Interface[this.exportTypeKey].enablecard_findById(req).then(res=>{
                    if (res.status == 200) {
                        ...
                        if(res.data.bBegintime && res.data.bEndtime){
            ...
            handleSubmit(validInfo){
                this.$refs[validInfo].validate((valid) => {
                    if (valid) {
                        ...
                        Interface[this.exportTypeKey].enablecard_submit(req).then(res=>{
                            if (res.status == 200) {
