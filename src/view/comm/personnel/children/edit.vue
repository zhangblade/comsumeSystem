<template>
    ...
    <Layout class="router_view">
        ...
        <div class="adapt_wrap">
            <Content>
                <Form ref="itemData" :model="itemData" :label-width="120" :rules="formRules">
                    ...
                    <FormItem label="出生日期" prop="bBirthday"><DatePicker type="date" placeholder="请输入出生日期" v-model="itemData.bBirthday" @on-change="itemData.bBirthday=$event"></DatePicker></FormItem>
        ...
        <Modal v-model="showModal" title="图片裁剪" class-name="vertical-center-modal" :loading="true" :mask-closable="false" :width="873">
            <div class="cropper_module">
                ...
                <div class="cropper_preview">
                    <h5>预览{{`大小:${cropperImg.fileSize}k(${cropperImg.imgWidth}*${cropperImg.imgHeight})`}}</h5>
            ...
            <div slot="footer">
                ...
                <Button type="primary" :loading="isSubmitting" @click="handleOk()" style="margin-left: 8px;">确认</Button>
            </div>
        </Modal>
...
<script>
	...
	export default {
		...
		created(){
            this.itemData.bPerid = this.$route.query.bPerid
            ...
            getEditItemData(){
				...
				Interface[this.exportTypeKey].findById(req).then(res=>{
					if (res.status == 200) {
                        ...
                        this.groups.name = res.data.bGroupName;
                        this.pertypes.name = res.data.bTypename;
                        this.bPhotoArr = res.data.bPhotoList ? res.data.bPhotoList.split(',') : []
					}
                })
            },
            formatBrithday(){
                var d = new Date(this.itemData.bBirthday)
                ...
                this.$refs[validInfo].validate((valid) => {
                    if (valid) {
                        ...
                        Object.keys(this.itemData).forEach(key=>{
                            if(key == 'bBirthday'){
                                if(this.itemData.bBirthday){
                                    req[key] = this.formatBrithday()
                                }
                            }else if( key == 'bBegintime'){
                                req[key] = this.itemData.bBegintime.split(' 00:00:00')[0] + ' 00:00:00'
                            }else if( key == 'bEndtime'){
                                req[key] = this.itemData.bEndtime.split(' 00:00:00')[0] + ' 00:00:00'
                            }else {
                                req[key] = this.itemData[key]
                            }
                        })
